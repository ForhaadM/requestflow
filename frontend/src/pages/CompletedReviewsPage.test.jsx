...
beforeEach(() => {
  ...
  requestsApi.getAllRequests.mockResolvedValue([APPROVED_REQUEST])
...
describe('CompletedReviewsPage', () => {
  ...
  it('hides a review whose request was filtered out server-side', async () => {
  ...
  it('re-fetches requests with the search term once the debounce settles', async () => {
    render(<CompletedReviewsPage />)
    await waitFor(() => expect(screen.getByText('#2')).toBeInTheDocument())
